...
/**
 * Dither Reveal — Originkit. A WebGL panel that renders its image as a moving
 * dither pattern and dissolves back to full colour under the pointer.
 *
 * Ported from `DitherReveal.jsx` in the design source; the shaders are unchanged.
 */
...
export type DitherStyle = "bayer8" | "lines" | "noise";

const DITHER_INDEX: Record<DitherStyle, number> = { bayer8: 0, lines: 1, noise: 2 };

export interface DitherRevealProps {
  image?: string | { src?: string; url?: string };
  fit?: "cover" | "contain";
  /** 0–100; which band of a tall image stays in frame. */
  focusY?: number;
  ditherStyle?: DitherStyle;
  /** 1–20. */
  dotSize?: number;
  /** 20–600, in CSS pixels. */
  revealRadius?: number;
  /** 0–100. */
...
/** Uniform values, already clamped and normalised for the shader. */
interface Settings {
  fit: number;
  focusY: number;
  ditherStyle: DitherStyle;
...
function imageURL(value: DitherRevealProps["image"]): string {
  if (!value) return "";
  if (typeof value === "string") return value;
  return value.src ?? value.url ?? "";
}

function toSettings(p: Required<Omit<DitherRevealProps, "image" | "style">>): Settings {
  const speed = p.wave ? clamp(p.waveSpeed, 1, 100, DEFAULTS.waveSpeed) / 100 : 0;
  const amplitude = speed * 0.4;
  return {
    fit: p.fit === "contain" ? 1 : 0,
    focusY: clamp(p.focusY, 0, 100, DEFAULTS.focusY) / 100,
    ditherStyle: p.ditherStyle,
    pixelSize: clamp(p.dotSize, 1, 20, DEFAULTS.dotSize) / 2,
    revealRadius: clamp(p.revealRadius, 20, 600, DEFAULTS.revealRadius),
    revealSoftness: clamp(p.revealSoftness, 0, 100, DEFAULTS.revealSoftness) / 100,
    waveSpeed: speed,
    waveFrequency: clamp(p.waveDensity, 5, 100, DEFAULTS.waveDensity) / 10,
    waveAmplitude: amplitude,
    waveMargin: amplitude * 0.15,
  };
}

export function DitherReveal(props: DitherRevealProps) {
  const settings = toSettings({
    fit: props.fit ?? DEFAULTS.fit,
    focusY: props.focusY ?? DEFAULTS.focusY,
    ditherStyle: props.ditherStyle ?? DEFAULTS.ditherStyle,
    dotSize: props.dotSize ?? DEFAULTS.dotSize,
    revealRadius: props.revealRadius ?? DEFAULTS.revealRadius,
    revealSoftness: props.revealSoftness ?? DEFAULTS.revealSoftness,
    wave: props.wave ?? DEFAULTS.wave,
    waveSpeed: props.waveSpeed ?? DEFAULTS.waveSpeed,
    waveDensity: props.waveDensity ?? DEFAULTS.waveDensity,
  });
  ...
  // Prop changes reach the render loop through a ref, so tweaking a knob does
  // not tear down and rebuild the GL context.
  const liveRef = useRef(settings);
  liveRef.current = settings;

  const imgUrl = imageURL(props.image) || DEFAULT_IMAGE;
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // A host without CORS headers taints the upload — keep the
        // placeholder rather than tearing the effect down.
      }
    };
    img.onerror = () => console.warn("DitherReveal: image failed to load:", imgUrl);
    img.src = imgUrl;
