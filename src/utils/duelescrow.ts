 ...
 *
 * **This is NOT the artifact `contracts/DuelEscrow.sol` builds.** The source in
 * this repo is the USDC escrow: an `IERC20` constructor argument, a 4% rake to
 * a `treasury`, `open`/`join`/`settle`/`claimRake`, and a `MIN_STAKE` of 1_000
 * base units (0.001 USDC at 6dp). The contract at the address below was
 * deployed from Remix out of a different, native-ETH source — a payable
 * `stake(bytes32)`, no token, no rake, `winStake`/`loseStake` — which is why
 * the ABI beneath it says `payable` and denominates everything in wei.
 *
 * The two are read together nowhere, and must not be: this module and the
 * `/test` console it feeds are an operator's window onto one live deployment,
 * not a description of the source in `contracts/`. Whether the repo should also
 * carry that ETH source is an open question for the owner, flagged rather than
 * decided by the gamestake merge.
 */
export const BASE_SEPOLIA_DUEL_ESCROW_ADDRESS = "0xc683A484FC42eD99c48ad8E19F841388536deB3E" as const;

// One home for the chain id — `SIGNING_CHAIN_ID` in src/data/wallet.ts.
export { BASE_SEPOLIA_CHAIN_ID } from "../data/base-network.ts";
