...
/** The position, held through a compressed window of the tape. */
export function Tape(p: TapeProps) {
  const nLegs = p.legs.length;
  const progress = (p.pos / TAPE_LEN) * 100;
  const firstSym = p.arena[0];
  const hits = p.legs.filter((l) => legState(l, p.salt, p.pos).won).length;
  const cols = Math.min(3, Math.max(2, p.arena.length));

  const raceNote = p.raceDone
    ? "Tape complete. " +
      (hits === nLegs
        ? "Every leg closed beyond its line."
        : `${hits} of ${nLegs} legs landed — settle to see what that pays.`)
  ...
  return (
    <div style={sx("padding:24px 28px;max-width:1720px;margin:0 auto")}>
      <div style={sx("display:flex;align-items:center;gap:16px;margin-bottom:18px")}>
        ...
        <span style={sx(`font:700 18px/1 ${MONO};color:${C.accent}`)}>{p.potentialLabel}</span>
      </div>
      ...
      <div style={sx("display:grid;grid-template-columns:280px minmax(0,1fr);gap:18px;align-items:start")}>
        <div
          style={sx(
            "border:1px solid rgba(99,102,241,.4);border-radius:12px;padding:16px;" +
              "background:linear-gradient(180deg,rgba(99,102,241,.1),#0f0f11 40%)",
          )}
        >
          <div
            style={sx(
          ...
          >
            <span style={sx(`font:700 13px/1 ${SANS}`)}>Your position</span>
              ...
              return (
                ...
                >
                  <div style={sx("display:flex;align-items:center;justify-content:space-between")}>
                    <span style={sx(`font:700 12px/1 ${MONO}`)}>
                      {l.sym}
                      <span style={sx(`margin-left:8px;font:500 8.5px/1 ${MONO};letter-spacing:.1em;color:${C.dim}`)}>
                        {l.tier}
                      </span>
                    </span>
                    <span
                  ...
                  <div style={sx(`margin-top:7px;font:400 10px/1.4 ${MONO};color:${C.dim}`)}>
                    {conditionText(l)}
                  </div>
                  <div style={sx("margin-top:6px;display:flex;align-items:baseline;justify-content:space-between")}>
        ...
        <div style={sx(`display:grid;grid-template-columns:repeat(${cols},minmax(0,1fr));gap:12px`)}>
          {p.arena.map((sym) => {
            ...
            const held = p.legs.filter((l) => l.sym === sym);
                    ...
                    <span
                      ...
                      style={sx(
                        `font:700 8.5px/1 ${MONO};letter-spacing:.06em;padding:5px 6px;` +
                          `border-radius:5px;background:${C.indigo}26;color:${C.indigo}`,
                      )}
                    >
                      {l.tier} {l.dir === "over" ? "↑" : "↓"}
                      {l.t.toFixed(1)}%
