...
import { assertSigningChain, type WalletSource } from "../data/wallet.ts";
...
export function GameStakeTest({
  ...
  async function transact(label: string, method: string, args: unknown[], overrides?: unknown) {
    ...
    try {
      // The same refusal the nine desk paths use, at the same moment: the last
      // instant before a signer exists. `walletBlocker` above is a *label* — it
      // decides what the button says — and a label is not a guard. GameStake is
      // a Base Sepolia contract, so this assertion passes on the intended
      // chain; it is here so that this console cannot become the one path in
      // the app that reaches a signer without naming the chain it is on.
      assertSigningChain(wallet.identity.chainId, `${label} on GameStake`);
