...
**Shipped `e7e4141`:** `fullLadderSlice` now deals the expiry that fills the
...
**NOT done: `TIER_BANDS` is untouched**, deliberately. The venue was
unreachable (see the DNS block above) and the only capture in the tree —
`test/fixtures/orders.json` — is by its own provenance block "30 of 426 live
...
**The threshold that matters is not the score — it is `loud`.** `summarize`
sets `loud = prob < LOUD_BELOW (0.1)`. An all-SAFE slip goes loud once SAFE's
...
violet border, violet ODDS and ALL LAND (`ParlayPick.tsx:613,668,669`). The
safest slip in the game would render as the alarm. The fix is almost certainly
`LOUD_BELOW`, **not** the bands — but it is the owner's call, and it must be
made in the same change as the re-cut or the game will look broken.

`degeneracyScore` is `Π(1/p)`, so scaling every midpoint by `k` inflates an
n-leg score by `k^-n`: at k=0.6 that is ×2.78 (2 legs) to ×7.72 (4 legs).
`IF IT PAYS` already tops out at 26,666,667 pts today; a downward re-cut
multiplies that ceiling by `k^-4`.

**The two halves are NOT independent.** `slotsCoveredAt` buckets via `tierOf`,
so which expiry the grid deals is itself a function of `TIER_BANDS`.
Re-cutting the bands can change the selected expiry on the same snapshot —
**re-measure slot counts AFTER the re-cut**, never reuse a number measured
against the shipped bands. The "ETH 2 at front expiry vs 11 at 09-07" figure
above is exactly what must be re-measured.

Exhaustive list of places that hand-assert the old 75/55/35/15 and must be
edited alongside the constant: `parlay.ts` 62–78 and 142; `optionize.ts` 10–11
and 78–79; `test/parlay.test.ts` 44–99, 145, 151, 221; `test/app.test.tsx`
1123–1125 and **1699–1706 (a rendered `"35% chance"` assertion — it will fail
loudly)**; `test/detail.test.ts` 1028 and 1135. `test/detail.test.ts:1281-1282`
derives from the constant and needs no edit. `parlay.ts:170`
`TIER_MOVE.SAFE: 0.35` is tape geometry, not a probability — leave it.
